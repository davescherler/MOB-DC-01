...
class ViewController: UIViewController {
    ...
    @IBAction func pressPlay(sender: AnyObject) {
        ...
        updateLabel.text = self.game.dealCards() + self.game.firstHand()
    }
    
    @IBAction func takeAHit(sender: UITapGestureRecognizer) {
        updateLabel.text = self.game.hit()
    }
    
    @IBAction func stayAndShowHand(sender: UISwipeGestureRecognizer) {
        let gameOne = CardGame()
        updateLabel.text = self.game.stayAndShowHand()
    }
